...
describe('BookContext', () => {
  describe('bookReducer', () => {
    it('should add a new book', () => {
      const newBook = { id: 3, title: 'New Book', author: 'New Author', year: '2022', genre: 'Science Fiction' };
    ...
    it('should update an existing book', () => {
      const updatedBook = { id: 1, title: '1984 Updated', author: 'George Orwell', year: '1949', genre: 'Dystopian' };
      const action = { type: 'UPDATE_BOOK', payload: updatedBook };

      const state = bookReducer(initialBooks, action);

      expect(state).toContainEqual(updatedBook);
      expect(state.find(book => book.id === updatedBook.id).title).toBe('1984 Updated');
    });
  ...
  describe('BookProvider and useBookContext', () => {
    ...
    const BookProvider2 = ({ children }) => {
      ...
      expect(result.current.books.find(book => book.id === updatedBook.id).title).toBe('1984 Updated');
    });
